...
fn get_maybe(
    ...
    match res_operand {
        ...
        ResOperand::BinOp(BinOpOperand { op, a, b }) => match op {
            cairo_lang_casm::operand::Operation::Add => {
                let b_val = match b {
                    DerefOrImmediate::Immediate(b) => {
                        let val: u32 = b.value.clone().try_into().unwrap();
                        println!("val: {}", val);
                        let as_m31: M31 = val.into();

                        QM31::from_m31(as_m31, M31::zero(), M31::zero(), M31::zero()).into()
                    }
                    DerefOrImmediate::Deref(b) => memory[cell_ref_address(b, state)],
                };

                Some(memory[cell_ref_address(a, state)] + b_val)
            }

            cairo_lang_casm::operand::Operation::Mul => todo!("implmenent"),
        },
    }
}
