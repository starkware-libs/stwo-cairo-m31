...
fn assign_or_assert_operation(
    ...
    match (
        ...
        (Some(dest_val), Some(op1_val), Some(op2_val)) => {
            assert_eq!(dest_val, apply(op1_val, op2_val), "Assertion failed.");
        }
        ...
        (Some(dest_val), None, Some(op2_val)) => {
            memory.insert(op2_addr, deduce(dest_val, op2_val));
        }
        (Some(dest_val), Some(op1_val), None) => {
            memory.insert(op2_addr, deduce(dest_val, op1_val));
        }
...
fn assign_or_assert_operation_with_imm(
    ...
    match (memory.get(dest_addr), memory.get(op1_addr)) {
        (Some(dest_val), Some(op1_val)) => {
            assert_eq!(dest_val, apply(op1_val, immediate), "Assertion failed.");
        }
        (None, Some(op1_val)) => {
            memory.insert(dest_addr, deduce(op1_val, immediate));
        }
        (Some(dest_val), None) => {
            memory.insert(op1_addr, deduce(dest_val, immediate));
        }
        _ => panic!("Cannot deduce more than one operand"),
    };
}
