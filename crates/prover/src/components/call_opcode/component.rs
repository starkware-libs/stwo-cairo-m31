...
impl FrameworkEval for Eval {
    ...
    fn evaluate<E: EvalAtRow>(&self, mut eval: E) -> E {
        ...
        let [is_rel, base_reg] = std::array::from_fn(|_| eval.next_trace_mask());
        eval.add_constraint(is_rel.clone() * is_rel.clone() - is_rel.clone());
        eval.add_constraint(
            (base_reg.clone() - E::F::from(M31(2)))
                * (base_reg.clone() - E::F::one())
                * base_reg.clone(),
        );

        // Check instruction.
        let [off0] = std::array::from_fn(|_| eval.next_trace_mask());
        let opcode =
            E::F::from(INSTRUCTION_BASE) + is_rel.clone() + E::F::from(M31(2)) * base_reg.clone();
        eval.add_to_relation(RelationEntry::new(
            &self.memory_lookup,
            E::EF::one(),
            &[pc.clone(), opcode.clone(), off0.clone()],
        ));

        // Return pointers.
        [
            [ap, fp],                             // Return `fp`.
            [ap + E::F::one(), pc + E::F::one()], // Return `pc`.
        ]
        .iter()
        .map(|memory_entry| {
            eval.add_to_relation(RelationEntry::new(
        ...
        // Yield final state.
        let new_pc = pc + E::F::one(); // FIX
        let new_ap = ap + E::F::from(M31(2));
        let new_state = [new_pc, new_ap, new_ap];
        eval.add_to_relation(RelationEntry::new(
            &self.state_lookup,
            -E::EF::one(),
            &new_state,
