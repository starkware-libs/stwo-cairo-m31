    ...
    trace
        ...
        .for_each(|((row, opcode_input), lookup_data)| {
            ...
            *lookup_data.state[0] = [pc, ap, fp];
            ...
            // Decode insturction.
            let [opcode, reg, off, abs_dest] =
                memory_trace_generator.deduce_output(pc).into_packed_m31s();
            let imm = memory_trace_generator
                .deduce_output(pc + M31::one())
                .into_packed_m31s()[0];

            *lookup_data.memory[0] = [pc, opcode, reg, off, imm];

            let [op_type, off] = decode_opcode(INSTRUCTION_BASE, opcode, [2, 2]);
            ...
            *row[15] = maybe_inverse[3];
            ...
            let is_zero_flag = PackedM31::from_array(std::array::from_fn(|i| {
                if val[i].is_zero() {
                    M31(0)
                } else {
                    M31(1)
            ...
            *row[16] = is_zero_flag;
            ...
            let branch_target = Selector::select(&reg, [&pc_plus_imm, &abs_dest]);
            let mask = is_zero_flag.into_simd().simd_ne(Simd::splat(0));
            // NEW_PC[i] = is_zero_flag[i] ? pc[i] + 1 : branch_target[i]
            let new_pc = unsafe {
